...
# Sweeney et al., 2006, 2007; van den Hurk et al., 1998). Triangles denote the
# five longitudinal sites where An. farauti was collected (van den Hurk et al.,
# 2000). The two panels show the raw longitudinal trap counts at two
# representative sites (Cairns and Ninds Creek), linked by arrows to the dashed
# box enclosing the longitudinal survey sites.
# NOTE: the caption that appears in the paper lives in the manuscript .tex --
# update it there too.
# ------------------------------------------------------------------------------
...
obs <- read.csv(file.path(dir_data, "ab.csv"))  |> 
...
# Explicit, verified code -> name map keyed on the actual ab.csv$site codes.
# (Each site has two traps sharing a name; verified against site coordinates and
# habitat.) Eubenangee Swamp (E1/E2) is deliberately omitted: no An. farauti there.
site_lookup <- c(
  "BM1" = "Black Mountain Road", "BM2" = "Black Mountain Road",
  "K1"  = "Kuranda",             "K2"  = "Kuranda",
  "C1"  = "Cairns",              "C2"  = "Cairns",
  "RR1" = "Russell River",       "RR2" = "Russell River",
  "M1"  = "Ninds Creek",         "M2"  = "Ninds Creek"
  # E1, E2 = Eubenangee Swamp deliberately omitted (no An. farauti)
)

# Individual longitudinal trap coordinates (before aggregation). Used to (a) drop
# these locations from the presence/absence circles so triangles are not double-
# plotted over black presence circles (Option A), and (b) build one triangle/site.
ab_raw <- read.csv(file.path(dir_data, "ab.csv")) |>
  dplyr::filter(source == "andrew", region == "QLD", method == "T") |>
  dplyr::select(lon, lat, habitat, site) |>
  dplyr::distinct() |>
  dplyr::mutate(site_name = unname(site_lookup[site])) |>
  dplyr::filter(!is.na(site_name))                          # removes Eubenangee + any stray codes

# Remove the longitudinal trap locations from the presence/absence layer, so each
# time-series site is shown by a single clean triangle instead of a triangle
# stacked on a black presence circle.
obs <- dplyr::anti_join(obs, dplyr::select(ab_raw, lon, lat), by = c("lon", "lat"))

obs_sf <- st_as_sf(obs, coords = c("lon", "lat"), crs = 4326)

# One marker per site (mean trap location) for the triangles
ab <- ab_raw |>
  dplyr::group_by(site_name) |>                             # one marker per site
  dplyr::summarise(lon = mean(lon), lat = mean(lat),
                   habitat = dplyr::first(habitat), .groups = "drop") |>
  dplyr::mutate(site_name = factor(site_name,
                levels = c("Black Mountain Road","Kuranda","Cairns","Russell River","Ninds Creek")))

ab_sf <- st_as_sf(ab, coords = c("lon","lat"), crs = 4326)

# ---- Unified legend mapping ---------------------------------------------------
# Label the longitudinal (time-series) sites "Time series" in the legend so
# Absence / Presence / Time series read as one coherent "Survey type" category.
...
ts_raw <- read.csv(file.path(dir_data, "ab.csv")) |>
  dplyr::filter(source == "andrew", region == "QLD", method == "T") |>
  dplyr::mutate(
    site_name = unname(site_lookup[site]),
...
# Kept for reference only -- disabled with if (FALSE) so it is never drawn.
# The final figure uses just the main panel + the two time-series panels + arrows.
if (FALSE) {
...
}  # end if (FALSE) -- inset_zoom kept for reference, not drawn
...
final_map <- ggdraw(main_panel) + 
  ...
  draw_line(x = c(0.434, 0.47), y = c(0.435, 0.66), arrow = arr, colour = "grey30", linewidth = 0.4) +
  draw_line(x = c(0.448, 0.47), y = c(0.397, 0.46), arrow = arr, colour = "grey30", linewidth = 0.4)
