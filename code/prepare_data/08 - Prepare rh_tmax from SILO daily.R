...
# Compute rh_tmax and mean tmax over 21d prior of target date using SILO data
...
process_nc_file <- function(ncfile, indices) {
  ...
  results <- lapply(1:nrow(indices), function(i) {
    ...
    data.frame(
      ...
      date = dates,
      rh_tmax_value = rh_tmax[lon_idx, lat_idx, ]
    )
  })
...
merged_data <- merge(
  data,
  final_results[, .(date, UID, rh_tmax_value, rh_tmax_value_21d)], 
  by = c("date", "UID"),
  all.x = TRUE
)
...
# Rename
setnames(merged_data, "rh_tmax_value", "rh_tmax")
setnames(merged_data, "rh_tmax_value_21d", "mean_rh_tmax_21d")
...
write_fst(
  merged_data, 
  paste0(dir_output, "humidity/SILO/Farauti_ss_North_QLD_with_SILO_rh_tmax_1995-1997.fst")
)

write.csv(
  merged_data, 
  paste0(dir_output, "humidity/SILO/Farauti_ss_North_QLD_with_SILO_rh_tmax_1995-1997.csv"),
  row.names = FALSE
)
