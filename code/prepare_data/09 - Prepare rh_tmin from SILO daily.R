...
# Compute rh_tmin and mean tmin over 21d prior of target date using SILO data
...
process_nc_file <- function(ncfile, indices) {
  ...
  results <- lapply(1:nrow(indices), function(i) {
    ...
    data.frame(
      ...
      date = dates,
      rh_tmin_value = rh_tmin[lon_idx, lat_idx, ]
    )
  })
...
nc_files_years <- sub(".*?/([0-9]{4})\\..*", "\\1", nc_files)
...
duplicates <- final_results[duplicate_rows]
print(duplicates)

# Remove duplicate rows
if (dim(duplicates)[1] > 0) final_results <- unique(final_results)
rm(duplicates)
...
merged_data <- merge(
  data,
  final_results[, .(date, UID, rh_tmin_value, rh_tmin_value_21d)], 
  by = c("date", "UID"),
  all.x = TRUE
)
...
# Rename
setnames(merged_data, "rh_tmin_value", "rh_tmin")
setnames(merged_data, "rh_tmin_value_21d", "mean_rh_tmin_21d")
...
write_fst(
  merged_data, 
  paste0(dir_output, "humidity/SILO/Farauti_ss_North_QLD_with_SILO_rh_tmin_1995-1997.fst")
)

write.csv(
  merged_data, 
  paste0(dir_output, "humidity/SILO/Farauti_ss_North_QLD_with_SILO_rh_tmin_1995-1997.csv"),
  row.names = FALSE
)
