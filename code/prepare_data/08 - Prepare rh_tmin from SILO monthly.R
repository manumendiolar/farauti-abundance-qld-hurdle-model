## R Script for extracting and computing mean monthly rh_tmin values from SILO (Batch Processing) at specific locations
## Manuela, M.
## 04-03-2025
...
# Function to extract rh_tmin values & Fill NA at locations --------------------
process_nc_file <- function(ncfile, indices) {
  
  # Open NetCDF file
  nc <- nc_open(ncfile)
  
  # Extract time and `rh_tmin` values
  time <- ncvar_get(nc, "time")
  rh_tmin <- ncvar_get(nc, "rh_tmin")
  ...
  results <- lapply(1:nrow(indices), function(i) {
    ...
    rh_values <- sapply(1:length(dates), function(t) {
      val <- rh_tmin[lon_idx, lat_idx, t]
      
      # If NA, fill using mean of neighbors
      if (is.na(val)) {
        val <- fill_na_with_neighbors(rh_tmin, lon_idx, lat_idx, t)
      }
...
nc_files <- list.files(
  path = paste0(dir_input, "humidity/SILO/rh_tmin/"), 
  pattern = "\\.nc$", 
  full.names = TRUE
)
...
for (file in nc_files) {
  ...
  # Compute mean rh_tmin for each UID, year, and month
  file_results <- file_results[, .(rh_tmin_mean = mean(rh_tmin_value, na.rm = TRUE)), 
                               by = .(UID, year, month)]
  
  # Save intermediate results to disk (optional, for very large data)
  temp_output <- paste0(dir_output, "humidity/SILO/rh_tmin_1900-2024_andrew_locations_temp_results_")
...
temp_files <- list.files(path = paste0(dir_output, "humidity/SILO/"), 
                         pattern = "rh_tmin_1900-2024_andrew_locations_temp_results_.*\\.fst$", 
                         full.names = TRUE)
...
# Save Final Results
file_name <- paste0(dir_output, "humidity/SILO/rh_tmin_1900-2024_at_Farauti_numbers_AUS-Nigel_locations.fst")
...
# Read
# file_name <- paste0(dir_output, "humidity/SILO/rh_tmin_1985-2000_at_Farauti_numbers_AUS-Nigel_locations.fst")
...
month_col <- paste0("rh_tmin_mean_", selected_month)

# Extract rh_tmin_mean values for selected year
data_selected <- final_results[year == selected_year & month == selected_month, .(UID, lon, lat, rh_tmin_mean)]

# Identify missing values
data_selected[, missing := is.na(rh_tmin_mean)]
...
sum(is.na(data_selected$rh_tmin_mean))

# View summary of rh_tmin_mean values
summary(data_selected$rh_tmin_mean)

# Define color scale with 0 as the minimum
min_rh_tmin_mean <- min(data_selected$rh_tmin_mean, na.rm = TRUE)  # Force legend to start at 0
max_rh_tmin_mean <- max(data_selected$rh_tmin_mean, na.rm = TRUE)  # Keep max as the real max value

# Define color palette
library(viridis)
pal <- colorNumeric(palette = "viridis", domain = c(min_rh_tmin_mean, max_rh_tmin_mean), na.color = "gray")
...
map <- leaflet(data_selected) %>%
  ...
  addControl(
    html = paste0("<h4 style='text-align:left; font-weight:bold;'>RH tmin map: ",selected_year, "-", selected_month,"<br>Andrew's locations </h4>"), 
    position = "topleft"
  ) %>%  
  addCircleMarkers(
    ~lon, ~lat,
    color = ~pal(rh_tmin_mean),
    radius = 5,
    stroke = FALSE,
    fillOpacity = 0.8,
    popup = ~paste0("<b>UID:</b> ", UID, "<br>",
                    ...
                    "<b>RH tmin:</b> ", round(rh_tmin_mean, 2), "%")
  ) %>%
  addLegend(
    "bottomright", pal = pal, values = c(min_rh_tmin_mean, max_rh_tmin_mean),
    title = "RH tmin",
    opacity = 1
  )
...
saveWidget(
  map, 
  file = paste0(dir_output, "humidity/SILO/rh_tmin_1900-2024_map_at_Andrews_locations_",selected_year,"-",selected_month,".html"),
  selfcontained = TRUE
)
...
filtered_results_wide <- dcast(
  ...
  value.var = "rh_tmin_mean"
)

# Rename columns to reflect months
setnames(filtered_results_wide, paste0(1:12), paste0("rh_tmin_mean_", 1:12))

# Display the first few rows
head(filtered_results_wide)

# columns to keep
columns_to_keep <- c("UID", "year", paste0("rh_tmin_mean_", 1:12))  # Define required columns
...
setnames(data_mozzie_updated, 
         old = paste0("rh_tmin_mean_", 1:12), 
         new = paste0("rh_tmin_", 1:12))
# Check
head(data_mozzie_updated)
...
write_fst(
  data_mozzie_updated,
  path = paste0(dir_output, "humidity/Farauti_numbers_North_QLD-Andrew_with_SILO_monthly_mean_rh_tmin.fst"),
  compress = 50
)
