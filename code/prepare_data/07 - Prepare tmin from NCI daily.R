...
# Compute tmin and mean tmin over 21d prior of target date using NCI data
...
process_nc_file <- function(ncfile, indices) {
  ...
  results <- lapply(1:nrow(indices), function(i) {
    ...
    data.frame(
      ...
      date = dates,
      tmin_value = tmin[lon_idx, lat_idx, ]
    )
  })
...
nc_files <- list.files(
  path = paste0(dir_input, "temperature/NCI/Daily mean minimum temperature/"), 
  pattern = "\\.nc$", 
  full.names = TRUE
)
...
merged_data <- merge(
  data,
  final_results[, .(date, UID, tmin_value, tmin_value_21d)], 
  by = c("date", "UID"),
  all.x = TRUE
)
...
# Rename
setnames(merged_data, "tmin_value", "tmin")
setnames(merged_data, "tmin_value_21d", "mean_tmin_21d")
...
write_fst(
  merged_data, 
  paste0(dir_output, "temperature/Farauti_ss_North_QLD_with_NCI_tmin_1995-1997.fst")
)

write.csv(
  merged_data, 
  paste0(dir_output, "temperature/Farauti_ss_North_QLD_with_NCI_tmin_1995-1997.csv"),
  row.names = FALSE
)
