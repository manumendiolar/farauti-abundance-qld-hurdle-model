...
tif_folder <- paste0(dir_input, "precipitation/WorldClim/historical/wc2.1_cruts4.06_2.5m_prec_1960-2021/")
...
for (yr in unique(tif_info$year)[1]) {
  ...
  for (i in 1:nrow(year_files)[1]) {
    ...
    # Extract precipitation values for locations
    precip_values <- terra::extract(filled_raster, locations)
    
    # Identify rows where precipitation is missing
    #na_rows <- is.na(precip_values[, 2])
    
    # Convert to data.table and merge with year_data
    month_col <- paste0("precip_", year_files$month[i])  # e.g., "precip_1", "precip_2"
    year_data[, (month_col) := precip_values[, 2]]
  }
  toc()
  
  # Save intermediate results to disk (optional, for very large data)
  temp_output <- paste0(dir_output, "precipitation/Worldclim/wc2.1_cruts4.06_2.5m_prec_1960-2021_andrew_locations_temp_results_")
...
temp_files <- list.files(path = paste0(dir_output, "precipitation/Worldclim/"), 
                         pattern = "wc2.1_cruts4.06_2.5m_prec_1960-2021_andrew_locations_temp_results_.*\\.fst$", 
                         full.names = TRUE)
...
# Save Final Results
file_name <- paste0(dir_output, "precipitation/Worldclim/wc2.1_cruts4.06_2.5m_prec_1960-2021_at_Farauti_numbers_North_QLD-Andrew_locations.fst")
...
month_col <- paste0("precip_", selected_month)

# Extract precipitation values for selected year
data_selected <- final_results[year == selected_year, .(UID, lon, lat, precip = get(month_col))]

# Identify missing values
data_selected[, missing := is.na(precip)]
...
sum(is.na(data_selected$precip))

# View summary of precipitation values
summary(data_selected$precip)

# Define color scale with 0 as the minimum
min_precip <- 0  # Force legend to start at 0
max_precip <- max(data_selected$precip, na.rm = TRUE)  # Keep max as the real max value

# Define color palette
pal <- colorNumeric(palette = "PuRd", domain = c(min_precip, max_precip), na.color = "gray")
...
map <- leaflet(data_selected) %>%
  ...
  addControl(
    html = paste0("<h4 style='text-align:left; font-weight:bold;'>Precipitation map: ",selected_year, "-", selected_month,"<br>Andrew's locations </h4>"), 
    position = "topright"
  ) %>%  
  addCircleMarkers(
    ~lon, ~lat,
    ...
    popup = ~paste0("<b>UID:</b> ", UID, "<br>",
                    ...
                    "<b>Precipitation:</b> ", round(precip, 2), " mm")
  ) %>%
  addLegend(
    "bottomright", pal = pal, values = c(min_precip, max_precip),
...
saveWidget(
  map, 
  file = paste0(dir_output, "precipitation/Worldclim/wc2.1_cruts4.06_2.5m_prec_map_at_Andrew_locations_",selected_year,"-",selected_month,".html"),
  selfcontained = TRUE
)
...
# columns to keep
columns_to_keep <- c("UID", "year", paste0("precip_", 1:12))  # Define required columns
...
data_mozzie_updated[which(is.na(data_mozzie_updated$precip_6)), ]


# Save
write_fst(
  data_mozzie_updated,
  path = paste0(dir_output, "precipitation/Farauti_numbers_North_QLD-Andrew_with_worldclim_precipitation.fst"),
  compress = 50
)
